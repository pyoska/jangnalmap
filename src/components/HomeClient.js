...
export default function HomeClient({ initialMarkets = [] }) {
  ...
  // Debounce search input to prevent main-thread block
  useEffect(() => {
  ...
  // Performance: Lazy list rendering state
  ...
  // Read favorites & client update if initialMarkets empty
  useEffect(() => {
      ...
      <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/90 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-6 text-sm text-gray-500 font-medium">
            ...
            <Link href="/guide" className="hover:text-[#10B981] transition-colors">이용 백서 가이드</Link>
            <a href="#faq-section" className="hover:text-[#10B981] transition-colors">5일장 FAQ</a>
          </div>
        </div>
      </header>
